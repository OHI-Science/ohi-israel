...
# prep agriculture/aquaculture jobs + wages data:
# 'economies data.csv' is created from Anat's 'economies data.xlsx'


# setup
file_in  = file.path(dir_prep, 'economies data.csv')
...
head(jobs_by_israel); summary(jobs_by_israel)

# Option 1. 
write.csv(jobs_by_israel, file.path(dir_prep,'hotel_jobs_wrong_tmp.csv'), row.names=F, na='')
...
# Option 2. 
write.csv(jobs_by_2rgn, file.path(dir_prep,'hotel_jobs_right_tmp.csv'), row.names=F, na='')
